...
class Seller:
    ...
    def delivery_books(self, seller_id: str, order_id: str) -> int:
        ...
        #print(simplejson.dumps(json))
        url = urljoin(self.url_prefix, "delivery_books")
        headers = {"token": self.token}
        r = requests.post(url, headers=headers, json=json)
        return r.status_code

    def transfer_to_user(self, user_id: str, store_id: str, password: str, amount) -> int:
        json = {
            "user_id": user_id,
            "store_id": store_id,
            "password": password,
        ...
        #print(simplejson.dumps(json))
        url = urljoin(self.url_prefix, "transfer_to_user")
        headers = {"token": self.token}
        r = requests.post(url, headers=headers, json=json)
        return r.status_code