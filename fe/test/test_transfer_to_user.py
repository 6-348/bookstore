...
class TestTransfer:
    ...
    def pre_run_initialization(self):
        ...
        def test_ok(self):
            code = self.seller.transfer_to_user(self.seller_id,self.store_id,self.seller.password,self.total_price,self.seller.token)
            assert code == 200

        '''@exception： 401 password right? '''
        def test_authorization_error(self):
            password = self.seller.password+'_x'
            code = self.seller.transfer_to_user(self.seller_id,self.store_id,password,self.total_price,self.seller.token)
            assert code == 401

        '''@exception： 511 store_id exist?'''
        def test_storeid_error(self):
            store_id = self.store_id +"_ss"
            code = self.seller.transfer_to_user(self.seller_id,store_id,self.seller.password,self.total_price,self.seller.token)
            assert code == 511

        '''@exception： 519 balance suffient?'''
        def test_balance_exceed_error(self):
            amount = self.total_price + 20
            code = self.seller.transfer_to_user(self.seller_id,self.store_id,self.seller.password,amount,self.seller.token)
            assert code == 519
